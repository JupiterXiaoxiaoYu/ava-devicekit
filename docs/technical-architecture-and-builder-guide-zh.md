...
本文档描述当前 `` 框架的技术架构、后台能力、固件/硬件/UI/AI 的协作关系，以及开发者如何从 0 到 1 基于框架构建一个类似 Ava Box 的 AI 硬件产品。
...
| 路径 | 作用 |
|---|---|
| `backend/ava_devicekit/apps/base.py` | `HardwareApp` 最小接口：`boot()` 和 `handle()` |
| `backend/ava_devicekit/apps/registry.py` | app manifest 和 app 实例创建 |
| `backend/ava_devicekit/gateway/http_server.py` | HTTP gateway、admin API、device API、OTA API |
| `backend/ava_devicekit/gateway/websocket_server.py` | DeviceKit WebSocket gateway |
| `backend/ava_devicekit/gateway/firmware_compat.py` | 已部署设备的 WebSocket 协议适配桥；内部兼容层，不是框架主叙事 |
| `backend/ava_devicekit/gateway/runtime_manager.py` | 多设备 session、outbox、connection、event log |
| `backend/ava_devicekit/control_plane/store.py` | 本地控制面：users/projects/customers/devices/plans/usage/runtime config |
| `backend/ava_devicekit/runtime/settings.py` | runtime 配置模型和 sanitized 输出 |
| `backend/ava_devicekit/providers/` | ASR、LLM、TTS provider 接口和内置实现 |
| `backend/ava_devicekit/adapters/` | chain adapter 接口、Solana adapter、adapter registry |
| `backend/ava_devicekit/streams/` | market stream adapter、polling/live stream runtime |
| `backend/ava_devicekit/services/` | developer backend service registry 和 allowlisted invoke |
| `backend/ava_devicekit/ota/` | firmware catalog、OTA 响应、firmware publish |
| `firmware/include/ava_devicekit_runtime.h` | C 侧硬件运行时接口 |
| `shared_ui/include/ava_devicekit_ui.h` | C 侧 shared UI / input / context 接口 |
| `schemas/` | manifest、screen payload、input event、context snapshot、action draft、device identity、telemetry、transport profile、developer service 合约 |
| `userland/` | 开发者模板：app、provider、adapter、hardware port、UI、runtime config |
| `reference_apps/ava_box/` | Ava Box reference app UI/产品资产边界 |
| `backend/ava_devicekit/apps/ava_box.py` | Ava Box app 路由和 reference app 主逻辑 |
| `backend/ava_devicekit/apps/ava_box_skills/` | Ava Box app 层交易、watchlist、portfolio、execution 逻辑 |
...
```bash
cd /mnt/c/Users/72988/Desktop/AVE/ava-devicekit

cp userland/env.example .env.local
./scripts/run-devicekit-local.sh
```

`.env.local` 是 DeviceKit 本地运行的唯一默认密钥入口，文件被 git ignore，不应提交。旧 runtime 的 `.env` 只能作为迁移密钥来源，不能作为 DeviceKit 启动入口。
...
```bash
PYTHONPATH=backend python3 -m ava_devicekit.cli init-app ./my-device --type depin
```
...
```bash
cp userland/runtime.example.json runtime.local.json
```
...
```bash
PYTHONPATH=backend \
python3 -m compileall -q backend examples tests
```
