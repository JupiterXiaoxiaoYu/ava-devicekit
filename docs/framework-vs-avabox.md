...
| Watchlist, portfolio, trading skills | Ava Box app | `backend/ava_devicekit/apps/ava_box_skills` | Product behavior owned by Ava Box, not DeviceKit core. |
| Real wallet/trade transaction construction | Ava Box app | `backend/ava_devicekit/apps/ava_box_skills/execution.py` | ESP32 is the physical confirmation surface; Ava Box can use server-managed proxy/custodial wallets or any custom `TradeExecutionProvider`, with credentials and signing kept server-side. |
