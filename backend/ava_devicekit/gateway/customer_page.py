...
*{box-sizing:border-box}html{scrollbar-color:#9b8354 #ead7ad}body{margin:0;min-height:100vh;color:var(--ink);background:radial-gradient(circle at 85% -4%,rgba(204,133,48,.38),transparent 22rem),radial-gradient(circle at 6% 72%,rgba(95,112,68,.28),transparent 26rem),linear-gradient(115deg,#f7eed8,#e9d3a9);font-family:Aptos,"IBM Plex Sans","Segoe UI",sans-serif}button,input{font:inherit}.shell{min-height:100vh;display:grid;grid-template-columns:1fr;align-items:start;gap:clamp(22px,4vw,54px);padding:clamp(22px,5vw,58px)}.shell:has(.panel.ready){grid-template-columns:minmax(0,1fr) 440px}.story{display:grid;align-content:start;gap:clamp(20px,3vw,34px);padding-top:clamp(4px,2vw,26px)}.mark{display:inline-flex;align-items:center;gap:10px;font-weight:950;letter-spacing:.12em;text-transform:uppercase;color:#745327}.dot{width:18px;height:18px;border-radius:50%;background:conic-gradient(from 30deg,var(--forest),var(--amber),var(--forest));box-shadow:0 0 0 6px rgba(204,133,48,.14)}h1{font-family:Georgia,"Times New Roman",serif;font-size:clamp(44px,7vw,92px);line-height:.88;letter-spacing:-.055em;margin:0;max-width:850px}.lead{font-size:clamp(17px,2vw,23px);line-height:1.45;color:#5d5c48;max-width:720px;margin:0}.steps{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;max-width:900px}.step{border-top:1px solid rgba(70,58,32,.25);padding-top:14px}.step b{display:block;font-family:Georgia,"Times New Roman",serif;font-size:24px}.step span{display:block;color:var(--muted);font-weight:720;line-height:1.45;margin-top:4px}.panel{align-self:start;background:rgba(255,249,234,.88);border:1px solid rgba(88,66,27,.18);border-radius:34px;box-shadow:var(--shadow);padding:18px;backdrop-filter:blur(10px)}.panel.locked{display:none}.panel.ready{display:block;animation:slideIn .28s ease-out}@keyframes slideIn{from{opacity:0;transform:translateX(18px)}to{opacity:1;transform:none}}.card{border:1px solid var(--line);border-radius:27px;background:linear-gradient(180deg,#fffaf0,#f6e8c9);padding:20px}.card+.card{margin-top:12px}.kicker{color:#8b642c;text-transform:uppercase;font-weight:950;font-size:12px;letter-spacing:.13em}.title{font-family:Georgia,"Times New Roman",serif;font-size:30px;line-height:1;margin:6px 0}.copy{color:var(--muted);font-weight:720;line-height:1.45;margin:0 0 14px}.form{display:grid;gap:10px}.purchase-card{margin-top:4px;border:1px solid rgba(88,66,27,.18);border-radius:34px;background:rgba(255,249,234,.76);box-shadow:var(--shadow);padding:18px;max-width:980px}.purchase-row{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;align-items:end}.purchase-card .btn.primary{width:auto;min-width:210px}.purchase-card .activation-card{max-width:680px}.buy-line{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:10px}.field label{display:block;color:#766749;text-transform:uppercase;font-weight:950;font-size:11px;letter-spacing:.08em;margin:0 0 6px}.field input{width:100%;border:1px solid var(--line);border-radius:16px;background:#fffdf5;padding:12px 13px;color:var(--ink);outline:none}.field input:focus{border-color:var(--forest);box-shadow:0 0 0 4px rgba(95,112,68,.16)}.actions{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.btn{border:0;border-radius:999px;background:var(--forest);color:#fff4d8;padding:12px 16px;font-weight:950;cursor:pointer;text-decoration:none}.btn.primary{width:100%;justify-content:center;display:inline-flex}.btn.secondary{background:transparent;color:var(--forest);border:1px solid var(--line)}.btn:disabled{opacity:.55;cursor:not-allowed}.hidden{display:none!important}.status{min-height:22px;color:#5b6249;font-weight:820}.status.bad{color:var(--clay)}.wallet{display:none;font-family:"Cascadia Mono","SFMono-Regular",monospace;font-size:12px;overflow-wrap:anywhere;background:#fffdf5;border:1px solid var(--line);border-radius:16px;padding:10px}.wallet.active{display:block}.activation-card{display:none;border:1px solid var(--line);border-radius:20px;background:#fffdf5;padding:14px;margin-top:10px}.activation-card.active{display:block}.activation-code{font-family:Georgia,"Times New Roman",serif;font-size:30px;letter-spacing:.04em;margin:4px 0}.linkbox{font-family:"Cascadia Mono","SFMono-Regular",monospace;font-size:12px;overflow-wrap:anywhere;color:#5c563f}.check{display:flex;gap:9px;align-items:flex-start;color:var(--muted);font-weight:780;line-height:1.35}.check input{width:auto;margin-top:3px}.devices{display:grid;gap:10px}.device{border:1px solid var(--line);border-radius:19px;background:#fffdf5;padding:13px}.device-head{display:flex;align-items:center;justify-content:space-between;gap:10px}.device b{font-family:Georgia,"Times New Roman",serif;font-size:20px}.pill{display:inline-flex;border-radius:999px;padding:4px 8px;background:#e8dcc0;color:#3d3523;font-size:12px;font-weight:950}.pill.ok{background:#dce9c8;color:#2f5224}.meta{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:10px;color:var(--muted);font-size:13px;font-weight:760}.empty{border:1px dashed var(--line);border-radius:18px;padding:14px;color:var(--muted);font-weight:760}.fine{font-size:12px;color:#82745a;line-height:1.45;margin-top:12px}@media(max-width:860px){.shell,.shell:has(.panel.ready){grid-template-columns:1fr;padding:18px}.story{gap:18px;padding-top:0}.steps{grid-template-columns:1fr}.panel{align-self:stretch}.purchase-row{grid-template-columns:1fr}.purchase-card .btn.primary{width:100%}.meta{grid-template-columns:1fr}h1{font-size:clamp(42px,14vw,68px)}}
</style>
<body>
<main class="shell">
  <section class="story">
    ...
    <section class="purchase-card">
      ...
      <form id="demo-purchase-form" class="form">
        ...
        <div class="buy-line">
          ...
          <label class="check"><input type="checkbox" name="lock_wallet" id="lock_wallet"> Lock to connected wallet</label>
        </div>
  ...
  <aside class="panel locked" id="activation-panel">
    <section class="card">
      ...
      <form id="wallet-login-form" class="form">
        ...
        <button class="btn secondary hidden" type="button" id="switch-wallet">Switch wallet</button>
...
const qs=new URLSearchParams(location.search);if(qs.get('activation_code')){$('#activation_code').value=qs.get('activation_code');showActivationPanel()}if(qs.get('app_id'))$('#app_id').value=qs.get('app_id');if(qs.get('app_id'))$('#purchase_app_id').value=qs.get('app_id');
function setStatus(id,msg,bad=false){const el=$(id);el.textContent=msg||'';el.classList.toggle('bad',!!bad)}
function showActivationPanel(){const panel=$('#activation-panel');panel.classList.remove('locked');panel.classList.add('ready')}
function setWalletConnected(connected){$('#connect-wallet').classList.toggle('hidden',!!connected);$('#switch-wallet').classList.toggle('hidden',!connected)}
function publicPrefix(){const m=location.pathname.match(/^\/ava-devicekit(?=\/|$)/);return m?m[0]:''}function publicPath(path){if(!path||!path.startsWith('/'))return path;const base=publicPrefix();return base&& !path.startsWith(base+'/')?base+path:path}async function api(path,opts={}){const headers={...(opts.headers||{})};if(state.token)headers.Authorization='Bearer '+state.token;const res=await fetch(publicPath(path),{...opts,headers});const text=await res.text();let body={};try{body=text?JSON.parse(text):{}}catch(e){body={ok:false,error:text}}if(!res.ok||body.ok===false)throw new Error(body.error||res.statusText);return body}
function renderProfile(body){state.profile=body;const c=body.customer||{};if(c.wallet){state.wallet=c.wallet;$('#wallet').value=c.wallet;$('#wallet-view').textContent=c.wallet;$('#wallet-view').classList.add('active')}setWalletConnected(!!(state.wallet||c.wallet));$('#profile-summary').textContent=c.wallet?`Wallet ${short(c.wallet)}`:(c.email?`${c.display_name||c.email} - ${c.email}`:'Sign in to view your account.');const devices=body.devices||[];$('#device-list').innerHTML=devices.length?devices.map(d=>`<div class="device"><div class="device-head"><b>${esc(d.name||d.device_id)}</b><span class="pill ${['active','online_seen'].includes(d.status)?'ok':''}">${esc(d.status||'unknown')}</span></div><div class="meta"><span>ID ${esc(d.device_id||'-')}</span><span>App ${esc(d.app_id||'-')}</span><span>Board ${esc(d.board_model||'-')}</span><span>Firmware ${esc(d.firmware_version||'-')}</span></div></div>`).join(''):'<div class="empty">No device bound yet. Enter your activation code above.</div>'}
...
async function connectWallet(){const provider=window.solana;if(!provider||!provider.isPhantom)throw new Error('No Solana wallet found. Install Phantom or use a Solana wallet with signMessage.');const resp=await provider.connect();state.wallet=resp.publicKey.toString();$('#wallet').value=state.wallet;$('#wallet-view').textContent=state.wallet;$('#wallet-view').classList.add('active');setWalletConnected(true);return state.wallet}
function renderActivationCard(result){const card=result.activation_card||{},purchase=result.purchase||{};$('#demo-code').textContent=card.activation_code||result.activation_code||'-';$('#demo-device').textContent=`Device ${purchase.device_id||card.device_id||'-'} · App ${purchase.app_id||card.app_id||'-'} · ${purchase.customer_wallet?'wallet locked':'not wallet locked'}`;$('#demo-url').textContent=card.activation_url||'';$('#activation-card').classList.add('active');if(card.activation_code||result.activation_code)$('#activation_code').value=card.activation_code||result.activation_code;showActivationPanel()}
...
$('#switch-wallet').addEventListener('click',async()=>{localStorage.removeItem('ava_customer_token');state.token='';state.wallet='';state.profile=null;$('#wallet').value='';$('#wallet-view').textContent='';$('#wallet-view').classList.remove('active');setWalletConnected(false);$('#profile-summary').textContent='Sign in to view your account.';$('#device-list').innerHTML='<div class="empty">No session yet.</div>';try{const provider=window.solana;if(provider&&provider.disconnect)await provider.disconnect();setStatus('#login-status','Wallet session cleared. Connect another wallet.')}catch(err){setStatus('#login-status','Session cleared. Open your wallet extension and choose another account.')}});
...
setWalletConnected(false);if(state.token){showActivationPanel();me().catch(()=>{setWalletConnected(false)});}
</script>
</html>'''
